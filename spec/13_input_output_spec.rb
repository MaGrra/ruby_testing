...
describe NumberGame do
  ...
  describe '#game_over?' do
    ...
    context 'when user guess is not correct' do
      subject(:not_correct_guess) { described_class.new(3, '5')}
      # remove the 'x' before running this test
      it 'is not game over' do
        expect(not_correct_guess).not_to be_game_over
      end
    end
  end
  ...
  describe '#verify_input' do
    ...
    context 'when given invalid input as argument' do
      subject(:game_check_error) { described_class.new }

      it 'returns nil' do
        user_input = '10'
        verified_input = game_check_error.verify_input(user_input)
